...
export function PayrollClient({
  ...
  const getStaffNetSalary = (record: PayrollStaffRecord) => {
    ...
    return Math.max(
      0,
      base + ot + (record.totalFixedAllowances || 0) + record.totalCommissions + adj.allowances - adj.deductions
    );
  };
...
${adj.deductions > 0 ? `*B. POTONGAN (DEDUCTIONS):*\n- Potongan / Kasbon: Rp ${adj.deductions.toLocaleString("id-ID")}\n` : ""}----------------------------------------
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50/80 dark:bg-slate-950 text-slate-500 font-bold border-b border-slate-200 dark:border-slate-800">
                <tr>
                  ...
                  <th className="py-3.5 px-4">Bonus / Potongan (Rp)</th>
